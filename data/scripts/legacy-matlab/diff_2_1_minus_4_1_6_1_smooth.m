...
outputDir = fullfile(rootDir, 'difference_smooth_2_1_minus_4_1_6_1');
yColumn = 'ADC';
skipFirstRows = 20;
...
base = readCurve(dataDir, "2-1TCD1304*.txt", yColumn, skipFirstRows);
...
if ~isequal(base.x, curve4.x) || ~isequal(base.x, curve6.x)
    error('Pixel columns do not match among 2-1, 4-1, and 6-1.');
end
...
outTable = table(x, diff4Raw, diff6Raw, diff4Norm, diff6Norm, diff4Smooth, diff6Smooth, ...
    'VariableNames', {'Pixel', 'RawDiff_2_1_minus_4_1', 'RawDiff_2_1_minus_6_1', ...
    'NormalizedDiff_2_1_minus_4_1', 'NormalizedDiff_2_1_minus_6_1', ...
    'SmoothedNormalizedDiff_2_1_minus_4_1', 'SmoothedNormalizedDiff_2_1_minus_6_1'});
writetable(outTable, fullfile(outputDir, '2-1_minus_4-1_and_6-1_smoothed_normalized_data.csv'));

summary = table(["2-1 - 4-1"; "2-1 - 6-1"], ...
...
writetable(summary, fullfile(outputDir, '2-1_minus_4-1_and_6-1_summary.csv'));
...
title(ax, '2-1 difference curves after skipping first 20 points', ...
    'Interpreter', 'none');
legend(ax, 'Location', 'best', 'Interpreter', 'none');
set(ax, 'FontName', 'Arial', 'FontSize', 11, 'LineWidth', 1);

pngPath = fullfile(outputDir, '2-1_minus_4-1_and_6-1_smoothed_normalized_compare.png');
figPath = fullfile(outputDir, '2-1_minus_4-1_and_6-1_smoothed_normalized_compare.fig');
